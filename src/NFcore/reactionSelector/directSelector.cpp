...
double DirectSelector::refactorPropensitiesScaled(double scalelevel)
{
	Atot = 0;
	for(int r=0; r<n_reactions; r++) {
		Atot += reactionClassList[r]->update_a_scaled(double scalelevel);
	}
	return Atot;
}
...
double DirectSelector::getNextReactionClassScaled(ReactionClass *&rc, double scalelevel)
{
	double randNum = NFutil::RANDOM(Atot);
	...
	this->refactorPropensitiesScaled(scalelevel);
	return getNextReactionClass(rc);
